 ...
 *  - format ('html' || 'object')
 */
export async function compileAttrs(attrs, options) {


    
  console.assert(Array.isArray(attrs), 'Attrs should be an array');
  console.assert(
  ...
  console.assert(options && typeof options === 'object', 'Options should be an object');
  console.assert(
  ...
  if (classes.length) {
    ...
    return (options.format === 'html') ? (buf.length ? buf.join('+') : '""') :
  '{' + buf.join(',') + '}';
